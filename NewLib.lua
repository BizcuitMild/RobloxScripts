...
local Themes = {
    Names = {"Zyrion", "Dark", "Darker", "AMOLED", "Light", "Balloon", "SoftCream", "Aqua", "Amethyst", "Rose", "Midnight",
    ...
    Zyrion = {
		...
		AcrylicNoise = 0.94,
		TitleBarLine = Color3.fromRGB(255, 120, 75),
		Tab = Color3.fromRGB(255, 120, 75),
		...
		DialogHolderLine = Color3.fromRGB(255, 120, 75),
		DialogButton = Color3.fromRGB(35, 35, 35),
		DialogButtonBorder = Color3.fromRGB(55, 55, 55),
		DialogBorder = Color3.fromRGB(50, 50, 50),
		DialogInput = Color3.fromRGB(45, 45, 45),
		DialogInputLine = Color3.fromRGB(255, 120, 75),
...
local function setupMiniMessageSupport(object, properties)
    ...
    local function convertTextIfNeeded(text)
        ...
        if text:match("<[^>]+>") then
            local hasMiniMessagePattern = text:match("<%w+>") or text:match("<color:") or
...
function AcrylicBlur()
    local function createAcrylicBlur(distance)
        ...
        local function render()
            ...
            model.CFrame = CFrame.fromMatrix((topLeft3D + bottomRight3D) / 2, camera.XVector, camera.YVector,
                camera.ZVector)
...
Components.Element = (function()
    ...
    return function(Title, Desc, Parent, Hover, Options)
        ...
        if Hover then
            ...
            local Motor, SetTransparency = Creator.SpringMotor(Creator.GetThemeProperty("ElementTransparency"),
...
Components.Tab = (function()
    ...
    function TabModule:New(Title, Icon, Parent)
        ...
        function Tab:SelectSubTab(SubTabIndex)
            ...
            if PreviousSubTab > 0 and PreviousSubTab ~= SubTabIndex and self.SubTabs[PreviousSubTab] and
                self.SubTabs[SubTabIndex] then
    ...
    function TabModule:SelectTab(Tab)
        ...
        local ContainerSize = Window.ContainerHolder and Window.ContainerHolder.AbsoluteSize.X or
        ...
        if PreviousTab > 0 and PreviousTab ~= Tab and TabModule.Tabs[PreviousTab] and TabModule.Tabs[Tab] then
            ...
            if not OldContainer or not NewContainer or not OldTab.ContainerXMotor or
                not OldTab.ContainerTransparencyMotor or not NewTab.ContainerXMotor or
            ...
            for idx, Container in next, TabModule.Containers do
                if Container and idx ~= PreviousTab and idx ~= Tab then
                    ...
                    Container.Position = UDim2.fromOffset(0, 0)
                    Container.GroupTransparency = 0
                    if TabModule.Tabs[idx] and TabModule.Tabs[idx].ContainerXMotor and
        ...
        function NewDialog:Button(Title, Callback)
            ...
            for _, Btn in next, NewDialog.ButtonHolder:GetChildren() do
                if Btn:IsA("TextButton") then
                    Btn.Size = UDim2.new(1 / NewDialog.Buttons, -(((NewDialog.Buttons - 1) * 10) / NewDialog.Buttons),
                        0, 32)
                end
            end
...
Components.Notification = (function()
    ...
    function Notification:New(Config)
        ...
        }, {NewNotification.AcrylicPaint.Frame, NewNotification.Title, NewNotification.CloseButton,
        ...
        function NewNotification:ApplyTransparency()
            if Library.Theme == "Glass" and Library.UseAcrylic then
                ...
                if NewNotification.AcrylicPaint and NewNotification.AcrylicPaint.Frame and
                    NewNotification.AcrylicPaint.Frame.Background then
                    NewNotification.AcrylicPaint.Frame.Background.BackgroundTransparency = math.min(
                        notifBackgroundTransparency, 0.95)
                end
            end
        end
        ...
        local function Update()
            ...
            if not Textbox.Input:IsFocused() or Textbox.Input.TextBounds.X <= Reveal - 2 * PADDING then
                ...
                if Cursor ~= -1 then
                    ...
                    local width = TextService:GetTextSize(subtext, Textbox.Input.TextSize, Textbox.Input.Font,
    ...
    return function(Config)
                            ...
                            if section:IsA("Frame") and section.Name ~= "UIPadding" then
                                ...
                                if sectionContainer and sectionContainer:IsA("Frame") then
                                    ...
                                    if containerLayout then
                                        ...
                                        if searchTermForClosure == "" or hasVisibleChild then
                                            ...
                                            local containerPaddingTop = containerPadding and
                                                                            containerPadding.PaddingTop.Offset or 0
                                            local containerPaddingBottom = containerPadding and
                                                                               containerPadding.PaddingBottom.Offset or
                                                                               0
                                ...
                                if sectionLayout then
                                    ...
                                    local sectionContentSize =
                                        sectionLayout.AbsoluteContentSize.Y + sectionPaddingTop + sectionPaddingBottom
                ...
                if backgroundImageTransparency <= 0.1 then
                    ...
                    local function makeTransparent(obj)
                        ...
                        for _, child in ipairs(obj:GetChildren()) do
                            if not child:IsA("UICorner") and not child:IsA("UIGradient") and not child:IsA("UIStroke") and
                                not child:IsA("UIListLayout") and not child:IsA("UIPadding") then
        ...
        if Config.UserInfo then
            ...
            pcall(function()
                ...
                local content, isReady = Players:GetUserThumbnailAsync(Players.LocalPlayer.UserId,
            ...
            local titleText = tostring((Config.UserInfoTitle ~= nil and Config.UserInfoTitle) or
        ...
        Creator.AddSignal(Window.TitleBar.Frame.InputBegan, function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                ...
                if Window.Maximized then
                    StartPos = UDim2.fromOffset(Mouse.X - (Mouse.X * ((OldSizeX - 100) / Window.Root.AbsoluteSize.X)),
        ...
        Creator.AddSignal(UserInputService.InputChanged, function(Input)
            ...
            if (Input.UserInputType == Enum.UserInputType.MouseMovement or Input.UserInputType ==
                Enum.UserInputType.Touch) and Resizing then
                local Delta = Input.Position - ResizePos
                local StartSize = Window.Size

                local TargetSize = Vector3.new(StartSize.X.Offset, StartSize.Y.Offset, 0) + Vector3.new(1, 1, 0) * Delta
                local TargetSizeClamped = Vector2.new(math.clamp(TargetSize.X, 470, 2048),
        ...
        Creator.AddSignal(UserInputService.InputBegan, function(Input)
            if type(Library.MinimizeKeybind) == "table" and Library.MinimizeKeybind.Type == "Keybind" and
                ...
                pcall(function()
                    if Mobile then
                        ...
                        if mobileButton then
                            ...
                            if imageLabel then
                                imageLabel.Image = Window.Minimized and "rbxassetid://10734896384" or
        ...
        function Window:SetBackgroundImage(imageUrl, imageTransparency)
            if not Window.BackgroundImage then
                local imgTransparency = imageTransparency or Window.BackgroundImageTransparency or
...
ElementsTable.Dropdown = (function()
    ...
    function Element:New(Idx, Config)
        ...
        }, {SearchBar, DropdownScrollFrame, New("UICorner", {
            CornerRadius = UDim.new(0, 7)
        }), New("UIStroke", {
            ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
            ThemeTag = {
        ...
        local DropdownHolderCanvas = New("Frame", {
            ...
            Parent = Library.GUI,
            Visible = false
        }, {DropdownHolderFrame, New("UISizeConstraint", {
        ...
        local function RecalculateListSize()
            ...
            local estimatedContent = (visibleCount > 0) and
                                         (visibleCount * itemHeight + (visibleCount - 1) * padding + innerMargins +
                                             searchHeight) or (innerMargins + searchHeight)
            local maxHeight = 392
        ...
        Creator.AddSignal(UserInputService.InputBegan, function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                local mousePos = Input.UserInputType == Enum.UserInputType.MouseButton1 and
                                     Vector2.new(Mouse.X, Mouse.Y) or Input.Position
                local AbsPos, AbsSize = DropdownHolderFrame.AbsolutePosition, DropdownHolderFrame.AbsoluteSize
                local innerAbsPos, innerAbsSize = DropdownInner.AbsolutePosition, DropdownInner.AbsoluteSize

                local clickedInsideDropdown = mousePos.X >= AbsPos.X and mousePos.X <= AbsPos.X + AbsSize.X and
                                                  mousePos.Y >= AbsPos.Y and mousePos.Y <= AbsPos.Y + AbsSize.Y
                local clickedInsideInner =
                    mousePos.X >= innerAbsPos.X and mousePos.X <= innerAbsPos.X + innerAbsSize.X and mousePos.Y >=
                        innerAbsPos.Y and mousePos.Y <= innerAbsPos.Y + innerAbsSize.Y
        ...
        function Dropdown:BuildDropdownList()
            ...
            for Idx, Value in next, Values do
                ...
                }, {ButtonSelector, ButtonLabel, New("UICorner", {
                    CornerRadius = UDim.new(0, 6)
                })})
...
ElementsTable.Slider = (function()
    ...
    function Element:New(Idx, Config)
        ...
        local function calculateInputWidth(text)
            local textSize = game:GetService("TextService"):GetTextSize(text or "0", 12, Enum.Font.SourceSans,
                Vector2.new(1000, 14))
            local padding = 8
            local minWidth = 25
            local maxWidth = 80
        ...
        local function updateInputWidth(text, animate)
            ...
            if animate and math.abs(targetWidth - currentWidth) > 0.5 then
                currentWidthTween = TweenService:Create(SliderInput, TweenInfo.new(0.15, Enum.EasingStyle.Quart,
        ...
        Creator.AddSignal(UserInputService.InputChanged, function(Input)
            if Dragging then
                ...
                if position then
                    local SizeScale = math.clamp((position.X - SliderRail.AbsolutePosition.X) /
        ...
        Creator.AddSignal(SliderRail.InputBegan, function(Input)
            if Input.UserInputType == Enum.UserInputType.Touch then
                Dragging = true
                local SizeScale = math.clamp((Input.Position.X - SliderRail.AbsolutePosition.X) /
                                                 SliderRail.AbsoluteSize.X, 0, 1)
                Slider:SetValue(Slider.Min + ((Slider.Max - Slider.Min) * SizeScale))
            end
        end)
        ...
        function Keybind:GetState()
            ...
            elseif Keybind.Mode == "Hold" then
                ...
                if Key == "MouseLeft" or Key == "MouseRight" then
                    return
                        Key == "MouseLeft" and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or
                            Key == "MouseRight" and
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                ...
                Event = UserInputService.InputBegan:Connect(function(Input)
                    ...
                    EndedEvent = UserInputService.InputEnded:Connect(function(Input)
                        if Input.KeyCode.Name == Key or Key == "MouseLeft" and Input.UserInputType ==
                            Enum.UserInputType.MouseButton1 or Key == "MouseRight" and Input.UserInputType ==
        ...
        Creator.AddSignal(UserInputService.InputBegan, function(Input)
            if not Picking and not UserInputService:GetFocusedTextBox() then
                if Keybind.Mode == "Toggle" then
                    ...
                    if Key == "MouseLeft" or Key == "MouseRight" then
                        if Key == "MouseLeft" and Input.UserInputType == Enum.UserInputType.MouseButton1 or Key ==
                            "MouseRight" and Input.UserInputType == Enum.UserInputType.MouseButton2 then
        ...
        local function CreateColorDialog()
            ...
            Creator.AddSignal(RedInput.Input.FocusLost, function(Enter)
                if Enter then
                    ...
                    local Success, Result = pcall(Color3.fromRGB, RedInput.Input.Text, CurrentColor["G"],
                        CurrentColor["B"])
                    if Success and typeof(Result) == "Color3" then
            ...
            Creator.AddSignal(GreenInput.Input.FocusLost, function(Enter)
                if Enter then
                    local CurrentColor = GetRGB()
                    local Success, Result = pcall(Color3.fromRGB, CurrentColor["R"], GreenInput.Input.Text,
                        CurrentColor["B"])
                    if Success and typeof(Result) == "Color3" then
            ...
            Creator.AddSignal(BlueInput.Input.FocusLost, function(Enter)
                if Enter then
                    local CurrentColor = GetRGB()
                    local Success, Result = pcall(Color3.fromRGB, CurrentColor["R"], CurrentColor["G"],
                        BlueInput.Input.Text)
                    if Success and typeof(Result) == "Color3" then
...
local Icons = {
    ...
    ["lucide-zy-rion"] = "rbxassetid://85565655226280",
    ["lucide-zy-rion-icon"] = "rbxassetid://87997792051678",
    ["lucide-zy-rionicon"] = "rbxassetid://132253200980782"
}
...
function Library:CreateMinimizer(Config)
    ...
    local iconAsset = "rbxassetid://85565655226280"
...
local MinimizeButton = New("TextButton", {
    ...
    Color = ColorSequence.new {ColorSequenceKeypoint.new(0, Color3.fromRGB(40, 40, 50)),
